...
RSpec.describe Downloader::FilenameUtils do
  describe '#rename_to_number' do
    ...
    it 'raises a FilenameError when number is nil' do
      expect { Downloader::FilenameUtils.rename_to_number("cat.jpg", nil) }.to raise_error(Downloader::FilenameError, "Missing file number")
    end

    it 'raises a FilenameError when filename is nil' do
      expect { Downloader::FilenameUtils.rename_to_number(nil, 1) }.to raise_error(Downloader::FilenameError, "Missing filename")
    end

    it 'raises a FilenameError when filename and number are nil' do
      expect { Downloader::FilenameUtils.rename_to_number(nil, nil) }.to raise_error(Downloader::FilenameError, "Missing filename")
    end
  end
end
